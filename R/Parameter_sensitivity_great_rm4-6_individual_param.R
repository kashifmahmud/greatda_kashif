...
mass.quantify.rm4to6 = shift.output.bar
mass.quantify.rm4to6 = mass.quantify.rm4to6[,-1]
mass.quantify.rm4to6$Case = as.character(c("Baseline","VPD","Tempearature","SLA",
                                           "Vcmax and Jmax","Ea and Deltas",
                                           "g[1]","alpha and theta","Respiration",
                                           "allocations","Y","k"))
names(mass.quantify.rm4to6) = c("Case.rm4to6","biomass.rm4to6")

mass.quantify.rm4to6$mass.diff.rm4to6 = mass.quantify.rm4to6$biomass
mass.quantify.rm4to6 = mass.quantify.rm4to6 %>% mutate_at(3, funs(c(first(.), (. - first(.))[-1])) )

write.csv(mass.quantify.rm4to6, file = "output/final_mass_changes_rm4-6_individual.csv", row.names = FALSE)
