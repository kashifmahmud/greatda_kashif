...
Ct.group = data.frame(matrix(ncol = 7, nrow = length(treat.group)))
names(Ct.group) = c("GPP","Rg","Rm","Cs","Cr","Cw","Cf")
...
data.part = data.all[c("Date","Room","GPP")]
...
cpool = as.factor(c("Mleaf.modelled","Mwood.modelled","Mroot.modelled"))
...
for (v in 1:length(treat.group)) {
  ...
  Ct.group[v, c(5:7)] = data.part[which(data.part$treatment == treat.group[v] & data.part$Date == as.Date("2016-03-01")), 8:6] - data.part[which(data.part$treatment == treat.group[v] & data.part$Date == as.Date("2016-01-08")), 8:6]
  ...
  Ct.group$Cs[v] = data.part$Cs[which(data.part$treatment == treat.group[v] & data.part$Date == as.Date("2016-03-01"))]
  Ct.group$Rg[v] = Ct.group$GPP[v] - sum(Ct.group[v,c(3:7)])
}

Ct.fraction.group = Ct.group[, c(2:7)]
Ct.fraction.group[,] = Ct.fraction.group[,] / Ct.group[, 1] * 100
row.names(Ct.fraction.group) <- treat.group

Ct.group$treatment = treat.group
colnames(Ct.group) <- c("GPP (g C)", "Rg (g C)", "Rm (g C)", "Cs (g C)", "Cr (g C)", "Cw (g C)", "Cf (g C)", "Room")
Ct.group = Ct.group[,c(8,1,2,3,4,7,5,6)]
...
bb = barplot(as.matrix(t(Ct.fraction.group)), ylim=c(0, 107), ylab = "C Partitioning (%)", xlab = "Container size (L))",  
             col = cbPalette[1:8],legend = c(expression(R[g]),expression(R["m,tot"]),expression(C[s]),expression(C["s,r"]),expression(C["s,w"]),expression(C["s,f"])), 
