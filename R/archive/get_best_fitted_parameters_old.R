...
pslow <- function(alpha,PAR,theta,Tleaf,Vcmax,Jmax,Rdayfrac=NULL,g1,gsmodel=NULL,
                  VPD,CO2S,Rd,Tcorrect=F) {

  ps <- Photosyn(alpha=alpha,theta=theta,Vcmax=Vcmax,PPFD=PAR,Tleaf=Tleaf,Jmax=Jmax,
                 g1=g1,gsmodel="BBOpti",VPD=VPD,Ca=CO2S,Tcorrect=F,Rd=Rd)
  # ret <- ps$Aj - ps$Rd
  ret <- ps$ALEAF
  return(ret)
}
...
dat_ph<-merge(long_g1,aci_param[c(1,3,4,6,7)] ,by="Room")
...
tres_jvr<-read.csv("Parameters/great_jvr_growth_t.csv") #JV ratio for three growth temperatures 18, 28.5 and 35.5
tres_jvr[2]<-NULL
...
for (i in 1:length(unique(dat_c1$Room))) {
  fitalpha <- nls(Photo ~ pslow(alpha, theta,PAR=PARi,Tleaf=Tleaf,Vcmax,Jmax=round(JVr,3)*Vcmax,g1=g1,VPD=VpdL,CO2S=CO2S,Rd=Rd*.7,Tcorrect = F),
                  start=list(alpha=0.2,Vcmax=120,theta=.3), data=datlist[[i]], trace=TRUE)
...
vj_c1<-data.frame(alpha,alpha.se,theta,theta.se,Vcmax25,Vcmax25.se,Room,tair)
vj_c1$Campaign<-1
...
write.csv(phys_params_all,file = "Parameters/great_alpha_vcmax_jmax_g1.csv",row.names=FALSE,sep=",") 

phys_params_all[c("Tgrowth","Vcmax25","Vcmax25.se","Jmax25")]  <-NULL
...
par(mfrow=c(1,2))
plot(dat_c1$Photo,pslow(alpha[dat_c1$Room],theta=theta[dat_c1$Room],dat_c1$PARi,dat_c1$Tleaf,Vcmax=Vcmax25[dat_c1$Room],dat_c1$JVr*Vcmax25[dat_c1$Room],
                        g1=dat_c1$g1,VPD=dat_c1$VpdL,CO2S=dat_c1$CO2S,Rd=dat_c1$Rd),
     col=dat_c1$Room,xlab="Photo (Meas)",ylab="Photo (Pred)",xlim=c(0,30),ylim=c(0,30))
abline(0,1)

# both vs temperature
with(dat_c1,plot(Tleaf,Photo,col=Room))
with(dat_c1,points(Tleaf,pslow(alpha[dat_c1$Room],theta=theta[dat_c1$Room],dat_c1$PARi,Rd=dat_c1$Rd,dat_c1$Tleaf,Vcmax=Vcmax25[dat_c1$Room],dat_c1$JVr*Vcmax25[dat_c1$Room],
                               g1=dat_c1$g1,VPD=dat_c1$VpdL,CO2S=dat_c1$CO2S),col=dat_c1$Room,pch=19))

legend("topright",legend=unique(dat_c1$Room),col=unique(dat_c1$Room),pch=19)
...
for (i in 1:length(unique(dat_c2$Room))){
  fitalpha <- nls(Photo ~ pslow(alpha, theta,PAR=PARi, Tleaf=Tleaf,Vcmax,Jmax=round(JVr,2)*Vcmax,g1=g1,VPD=VpdL,CO2S=CO2S,Rd=Rd),
                  start=list(alpha=.3,Vcmax=70,theta=.3), data=datlist_c2[[i]], trace=TRUE)
...
vj_c2<-data.frame(alpha,alpha.se,theta,theta.se,Vcmax25,Vcmax25.se,Room,tair)
vj_c2$Campaign<-2
...
# plots
# modelled vs measured
plot(dat_c2$Photo,pslow(alpha[dat_c2$Room],theta=theta[dat_c2$Room],dat_c2$PARi,dat_c2$Tleaf,Vcmax=Vcmax25[dat_c2$Room],dat_c2$JVr*Vcmax25[dat_c2$Room]
                        ,g1=dat_c2$g1,VPD=dat_c2$VpdL,CO2S=dat_c2$CO2S,Rd=dat_c2$Rd),
     col=dat_c2$Room,xlab="Photo (Meas)",ylab="Photo (Pred)",xlim=c(0,30),ylim=c(0,30))
abline(0,1)

# both vs temperature
with(dat_c2,plot(Tleaf,Photo,col=Room))
with(dat_c2,points(Tleaf,pslow(alpha[dat_c2$Room],dat_c2$PARi,theta=theta[dat_c2$Room],dat_c2$Tleaf,
                               Vcmax=Vcmax25[dat_c2$Room],dat_c2$JVr*Vcmax25[dat_c2$Room],
                               g1=dat_c2$g1,VPD=dat_c2$VpdL,CO2S=dat_c2$CO2S,Rd=dat_c2$Rd),col=dat_c2$Room,pch=19))

legend("topright",legend=unique(dat_c2$Room),col=unique(dat_c2$Room),pch=19)
...
with(vcmax_dat,plot(tair,Vcmax25,col=COL[Campaign],pch=16))
with(vcmax_dat,plot(tair,alpha,col=COL[Campaign],pch=16))

# summaryBy(Photo~Room+Campaign+LightFac,data=avt.l)

#---------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------
mean(subset(vcmax_dat,vcmax_dat$theta>0)$theta)
#-----------------------------------------------------------------------------
#-- 
# #-- get initial values of Vcmax and Jmax assuming they linearly follow the trend between 2/3 to 2/26
# 
# Vcmax25<-Jmax25<-c()
# 
# for(i in 1:length(unique(vcmax_dat$Room))){
#   
#   rate_v<-(vcmax_dat[vcmax_dat$Date==as.Date("2016-02-26") & vcmax_dat$Room==i,]$Vcmax25-
#              vcmax_dat[vcmax_dat$Date==as.Date("2016-02-03") & vcmax_dat$Room==i,]$Vcmax25)/23
#   
#   rate_j<-(vcmax_dat[vcmax_dat$Date==as.Date("2016-02-26") & vcmax_dat$Room==i,]$Jmax25-
#              vcmax_dat[vcmax_dat$Date==as.Date("2016-02-03") & vcmax_dat$Room==i,]$Jmax25)/23
#   
#   
#   
#   Vcmax25[[i]]<-subset(bioch,bioch$Date=="2016-02-03" & bioch$Room==i)$Vcmax25+(rate_v*-1*27)
#   Jmax25[[i]]<-subset(bioch,bioch$Date=="2016-02-03" & bioch$Room==i)$Jmax25+(rate_j*-1*27)
#   
# }
# 
# vcj_in<-data.frame(Vcmax25,Jmax25)
# vcj_in$Room<-c(1:6)
# vcj_in$Date<-as.Date("2016-01-07")
# 
# 
# vcmax_dat<-rbind.fill(vcmax_dat,vcj_in)
# 
# 
# 
# 
...
write.csv(great_aci_params[c("Room","Vcmax25","Jmax25","Date")],file = "Parameters/great_vcmax_jmax.csv",row.names=FALSE,sep=",")
...
fitalpha <- nls(Photo ~ pslow(alpha, theta,PAR=PARi, Tleaf=Tleaf,Vcmax,Jmax=JVr*Vcmax,g1=g1,VPD=VpdL,CO2S=CO2S,Rd=Rd),
                start=list(alpha=0.3,Vcmax=120,theta=.5), data=datlist_c2[[1]], trace=TRUE)
resid(fitalpha)
