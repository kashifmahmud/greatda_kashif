...
# phy.data<-read.csv("Parameters/great_alpha_g1_ea_dels.csv")[c(1,2,3,12:16)]

phy.data<-read.csv("Parameters/great_alpha_g1_ea_dels.csv")[c(1,2,3)]
phy.data$Date<-NULL


# add alpha

al<-read.csv("Parameters/great_alpha_long_term_nls_method.csv")
phy.data<-merge(al,phy.data,by="Room")
...
bioch<-read.csv("Parameters/temperature_response_parameters.csv")[c(1:4,9:11)]
...
photo_gr<-with(metwithphy,Photosyn(VPD=VPD, Ca=400,Tleaf=Tair,PPFD=PAR,gsmodel = "BBOpti", g1=g1,alpha = alpha,
                                   theta = 0.3, Jmax =Jmax25, Vcmax =Vcmax25,delsC=delsV*10^3,delsJ=delsJ*10^3,EaV=EaV*10^3,EaJ=EaJ*10^3,
                                   Rd=0))
...
photom_15min$Rdark<-ifelse(photom_15min$period=="Day",with(photom_15min,R_leaf*2^((Tair-25)/10))*0.7, with(photom_15min,R_leaf*2^((Tair-25)/10))) # units gC gC-1 s-1
