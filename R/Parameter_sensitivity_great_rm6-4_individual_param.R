# This code performs sensitivity analysis with individual parameter shifting from Room 4 (25.8 degree C) to Room 6 (35.5 degree C)
# and make Figure 
#-------------------------------------------------------------------------------------
...
q=0 # Case 0
data.attrib.set = subset(data.attrib, Room %in% c(6)) # Start with Room 4 to test the parameter sensitivity
...
# 
# png("output/Figure_parameter_shifting_rm4-6_individual_linegraphs.png", units="px", width=3000, height=3500, res=300)
...
# 
# png("output/Figure_parameter_shifting_rm4-6_individual_barchart.png", units="px", width=1000, height=800, res=200)
# plot.shift[[17]]
# dev.off()
# 
# png("output/Figure_parameter_shifting_rm4-6_individual.png", units="px", width=3000, height=4000, res=300)
# lay <- rbind(c(1,2,13,13),c(3,4,13,13),c(5,6,14,14),c(7,8,14,14),c(9,10,15,15),c(11,12,15,15),c(16,16,17,17),c(16,16,17,17))
# grid.arrange(grobs = plot.shift, layout_matrix = lay)
# dev.off()
# 
# png("output/Figure_parameter_shifting_barchart_individual.png", units="px", width=1500, height=800, res=150)
...
mass.quantify.rm6to4 = shift.output.bar
mass.quantify.rm6to4 = mass.quantify.rm6to4[,-1]
mass.quantify.rm6to4$Case = as.character(c("Baseline","VPD","Tempearature","SLA",
                              "Vcmax and Jmax","Ea and Deltas",
                              "g[1]","alpha and theta","Respiration",
                              "allocations","Y","k"))
names(mass.quantify.rm6to4) = c("Case.rm6to4","biomass.rm6to4")

mass.quantify.rm6to4$mass.diff.rm6to4 = mass.quantify.rm6to4$biomass
mass.quantify.rm6to4 = mass.quantify.rm6to4 %>% mutate_at(3, funs(c(first(.), (. - first(.))[-1])) )

write.csv(mass.quantify.rm6to4, file = "output/final_mass_changes_rm6-4_individual.csv", row.names = FALSE)

mass.quantify.rm1to4 <- read.csv("output/final_mass_changes_rm1-4_individual.csv")
mass.quantify.rm4to1 <- read.csv("output/final_mass_changes_rm4-1_individual.csv")
mass.quantify.rm4to6 <- read.csv("output/final_mass_changes_rm4-6_individual.csv")

mass.quantify = mass.quantify.rm1to4
mass.quantify[4:6] = mass.quantify.rm4to1
mass.quantify[7:9] = mass.quantify.rm4to6
mass.quantify[10:12] = mass.quantify.rm6to4
mass.quantify[c(2,4,5,7,8,10,11)] = NULL
names(mass.quantify) = c("Parameter change", "18C >> 28.5C", "28.5C >> 18C", "28.5C >> 35.5C", "35.5C >> 28.5C")
mass.quantify$Parameter_change = as.character(mass.quantify$Parameter_change)
write.csv(mass.quantify, file = "output/final_mass_changes_individual.csv", row.names = FALSE)
