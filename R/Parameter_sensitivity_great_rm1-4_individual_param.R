...
mass.quantify.rm1to4 = shift.output.bar
mass.quantify.rm1to4 = mass.quantify.rm1to4[,-1]
mass.quantify.rm1to4$Case = as.character(c("Baseline","VPD","Tempearature","SLA",
                                           "Vcmax and Jmax","Ea and Deltas",
                                           "g[1]","alpha and theta","Respiration",
                                           "allocations","Y","k"))
names(mass.quantify.rm1to4) = c("Case.rm1to4","biomass.rm1to4")

mass.quantify.rm1to4$mass.diff.rm1to4 = mass.quantify.rm1to4$biomass
mass.quantify.rm1to4 = mass.quantify.rm1to4 %>% mutate_at(3, funs(c(first(.), (. - first(.))[-1])) )
# rownames(mass.quantify.rm1to4) = c("Baseline (at 18 C)","+ VPD","+ Tempearature","+ SLA",
#                             "+ Vcmax and Jmax","+ Ea and Deltas",
#                             "+ g[1]","+ alpha and theta","+ Respiration",
#                             "+ allocations","+ Y","+ k at 28.5 C")

write.csv(mass.quantify.rm1to4, file = "output/final_mass_changes_rm1-4_individual.csv", row.names = FALSE)
#-------------------------------------------------------------------------------------
# 
# 
# #-------------------------------------------------------------------------------------
