...
# title = as.character(c("Room#1","Room#2","Room#3","Room#4","Room#5","Room#6"))
  plot[[4]] = ggplot(data = data.set.final.melt, aes(x = Date, y = sla, group = as.factor(Room), colour=as.factor(Room))) + 
    ...
    geom_errorbar(position=pd,aes(ymin=sla-sla_SE, ymax=sla+sla_SE), colour="light grey", width=2) +
...
plot[[3]] = ggplot(data = final.harvest.sla, aes(x = Room, y = sla, group = as.factor(Prov), colour=as.factor(Prov))) + 
  ...
  geom_errorbar(position=pd,aes(ymin=sla-sla_SE, ymax=sla+sla_SE), colour="light grey", width=0.2) +
...
plot[[1]] = ggplot(data = leaf.punch.sla, aes(x = Room, y = sla, group = as.factor(Prov), colour=as.factor(Prov))) + 
  ...
  # theme(plot.title = element_text(hjust = 0)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
plot[[1]] = plot[[1]] + guides(colour=FALSE)
...
plot[[4]] = plot[[4]] + 
  ...
  geom_errorbar(position=pd, data = sla.points, aes(ymin=sla-sla_SE, ymax=sla+sla_SE), colour="light grey", width=2)
...
plot[[4]] = plot[[4]] + 
  ...
  geom_errorbar(position=pd, data = int.harvest.1.mean, aes(ymin=sla-sla_SE, ymax=sla+sla_SE), colour="light grey", width=2)
...
plot[[4]] = plot[[4]] + 
  ...
  geom_errorbar(position=pd, data = int.harvest.2.mean, aes(ymin=sla-sla_SE, ymax=sla+sla_SE), colour="light grey", width=2) +
...
png("output/Figure_sla_great.png", units="px", width=3000, height=1500, res=250)
print (do.call(grid.arrange,  plot))
dev.off()

sla.harvest.all = rbind(sla.points,int.harvest.1.mean,int.harvest.2.mean)
sla.harvest.all$sla = sla.harvest.all$sla / 10000 / c1 # unit conversion cm2 g-1 to m2 gC-1
sla.harvest.all$sla_SE = sla.harvest.all$sla_SE / 10000 / c1 # unit conversion cm2 g-1 to m2 gC-1
write.csv(sla.harvest.all, "processed_data/sla.harvest.all.csv", row.names=FALSE)
