...
param.af <- matrix(c(0.5,0.55,1) , nrow=1, ncol=3, byrow=T) # Forcing af not to be negetive
param.as <- matrix(c(0.3,0.4,0.5) , nrow=1, ncol=3, byrow=T)
...
if (with.storage==F) {
  ...
  param.k <- matrix(c(0.3,0.35,0.4) , nrow=1, ncol=3, byrow=T)
  if (no.param > 1) {
