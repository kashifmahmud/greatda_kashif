...
mcmc.great <- function(chainLength, no.param.per.var, treat.group, with.storage, model.comparison, model.optimization) {
  ...
  for (v in 1:length(treat.group)) {
    ...
    for (z in 1:length(no.param.per.var)) {
      ...
      if (no.param.per.var < 5) {
        ...
        if (with.storage==T) {
          melted.param1 = reshape2::melt(param.daily[,c("k","Y","af","as","ar","Date")], id.vars="Date")
          melted.param2 = reshape2::melt(param.daily[,c("k_SD","Y_SD","af_SD","as_SD","ar_SD","Date")], id.vars="Date")
        } else {
          melted.param1 = reshape2::melt(param.daily[,c("Y","af","as","ar","Date")], id.vars="Date")
          melted.param2 = reshape2::melt(param.daily[,c("Y_SD","af_SD","as_SD","ar_SD","Date")], id.vars="Date")
        }
        ...
        melted.param1 = reshape2::melt(param.final[,c("k","Y","af","as","ar","Date")], id.vars="Date")
        melted.param2 = reshape2::melt(param.final[,c("k_SD","Y_SD","af_SD","as_SD","ar_SD","Date")], id.vars="Date")
        melted.param = data.frame(melted.param1$Date, melted.param1$variable, melted.param1$value, melted.param2$value)
        names(melted.param) = c("Date","variable","Parameter","Parameter_SD")
        melted.param$Date = as.Date(melted.param$Date)
        melted.param$treatment = treat.group[v]
      ...
      #     melted.output = reshape2::melt(output.final.set[,c("Mleaf.modelled","Mwood.modelled","Mroot.modelled","Sleaf.modelled","Date")], id.vars="Date")
      #     melted.data = reshape2::melt(data.set[ , c("Mleaf","Mwood","Mroot","Sleaf","Date")], id.vars="Date")
      #     melted.error = reshape2::melt(data.set[ , c("Mleaf_SD","Mwood_SD","Mroot_SD","Sleaf_SD","Date")], id.vars="Date")
      #   } else {
      #     names(output.final.set) = c("Mleaf.modelled","Mwood.modelled","Mroot.modelled","volume","Date")
      #     melted.output = reshape2::melt(output.final.set[,c("Mleaf.modelled","Mwood.modelled","Mroot.modelled","Date")], id.vars="Date")
      #     melted.data = reshape2::melt(data.set[ , c("Mleaf","Mwood","Mroot","Date")], id.vars="Date")
      #     melted.error = reshape2::melt(data.set[ , c("Mleaf_SD","Mwood_SD","Mroot_SD","Date")], id.vars="Date")
      #   }
      ...
      if (with.storage==T) { 
        ...
        melted.output = reshape2::melt(output.final[,c("Mleaf.modelled","Mwood.modelled","Mroot.modelled","Rm","Date")], id.vars="Date")
        melted.Cstorage = output.final[,c("Cstorage.modelled","Sleaf.modelled","Swood.modelled","Sroot.modelled","Date")]
        melted.data = reshape2::melt(data.set[ , c("LM","WM","RM","Date")], id.vars="Date")
        melted.error = reshape2::melt(data.set[ , c("LM_SE","WM_SE","RM_SE","Date")], id.vars="Date")
        melted.Cstorage$Date = as.Date(melted.Cstorage$Date)
  ...
  bic = data.frame(aic.bic[,c("logLi","bic","no.param","treatment")])
  # melted.aic.bic = reshape2::melt(aic.bic[,c(1:5)], id.vars=c("no.param"))
...
# various temporal scales (e.g. 1,2,...,121 days) to estimate Carbon pools (Cstorage,Cleaf,Cwood,Croot) and fluxes
#-------------------------------------------------------------------------------------
mcmc.great.rm6 <- function(chainLength, no.param.per.var, treat.group, with.storage, model.comparison, model.optimization) {
  ...
  for (v in 1:length(treat.group)) {
    ...
    for (z in 1:length(no.param.per.var)) {
      ...
      if (no.param.per.var < 5) {
        # This script initializes the parameter setting
        source("R/parameter_setting_great_rm6.R", local=TRUE) # initialize parameters
...
plot.Modelled.parameters.great <- function(result,with.storage,treat.group) {
  ...
  cbPalette = c("navy","royalblue","skyblue","lawngreen","magenta","red")
  ...
  png("output/modelled_parameters_Figure.png", units="px", width=2000, height=2000, res=250)
  print (do.call(grid.arrange,  plot))
  dev.off()
}
...
plot.Modelled.biomass.great <- function(result,with.storage=T,treat.group) {
  ...
  png("output/modelled_biomass_Figure.png", units="px", width=1600, height=1300, res=220)
  print (do.call(grid.arrange,  plot))
  dev.off()
