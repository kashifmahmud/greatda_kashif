...
plot.Vcmax <- function(data.attrib.daily, iteration) { 
  ggplot(data = data.attrib.daily, aes(x = Date, y = Vcmax25,  group = Room, colour=factor(Room))) +
    geom_point(size=0.01) +
    geom_line(data = data.attrib.daily, aes(x = Date, y = Vcmax25,  group = Room, colour=factor(Room))) +
    ylab(expression(V[cmax25] ~ mu ~ mol~ m^{-2} ~ s^{-1})) +
    scale_colour_manual(name="", breaks=c("5", "1000"), labels=c("5L", "FS"), values=cbPalette[1:2]) +
    annotate("text", x = min(data.attrib.daily$Date), y = max(data.attrib.daily$Vcmax25)*0.98, size = font.size-7, label = paste(title[1])) +
    theme_bw() +
...
################ Figure 6B #####################
# Function to gerenate plot for Daily net C assimilation per unit leaf area (Cday) 
plot.Jmax <- function(data.attrib.daily, iteration) { 
  ggplot(data = data.attrib.daily, aes(x = Date, y = Jmax25,  group = Room, colour=factor(Room))) +
    geom_point(size=0.01) +
    geom_line(data = data.attrib.daily, aes(x = Date, y = Jmax25,  group = Room, colour=factor(Room))) +
    ylab(expression(J[max25]~"(g C "*d^"-1"*" "*leafarea^"-1"*")")) +
    scale_colour_manual(name="", breaks=c("5", "1000"), labels=c("5L", "FS"), values=cbPalette[1:2]) +
    annotate("text", x = min(data.attrib.daily$Date), y = max(data.attrib.daily$Jmax25)*0.98, size = font.size-7, label = paste(title[1])) +
    theme_bw() +
    theme(legend.position = c(0.85,0.85)) +
    theme(legend.title = element_blank()) +
    theme(legend.key = element_blank(), plot.margin=unit(c(0.25, 0.25, 0, 0.45), units="line")) +
